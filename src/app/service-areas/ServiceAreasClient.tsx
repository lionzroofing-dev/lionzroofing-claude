...
const CITIES = [
  "Miami", "Miami Beach", "Boca Raton", "Hallandale Beach", "North Miami",
  "Deerfield Beach", "Miami Gardens", "Aventura", "Hialeah", "Bal Harbour And Surfside",
  "Broward County", "North Miami Beach", "Fort Lauderdale", "West Palm Beach",
  "Hollywood", "Coral Gables", "Coconut Creek", "Sunrise", "Coral Springs",
  "Pompano Beach", "Pembroke Pines", "Miramar", "Margate",
];

const INTRO_PILLS = [
  "Miami", "Boca Raton", "Fort Lauderdale", "West Palm Beach",
  "Hollywood", "Broward County", "Surrounding Communities",
];
...
function FAQItem({ q, a }: { q: string; a: string }) {
  ...
  return (
    <div className="border-b border-gray-200 last:border-0">
      <button onClick={() => setOpen((v) => !v)} className="w-full flex items-center justify-between py-4 text-left gap-4 group">
        <span className="text-lionzNavy font-semibold text-sm md:text-base group-hover:text-lionzGold transition-colors">{q}</span>
        <ChevronDown size={18} className={`shrink-0 text-lionzGold transition-transform duration-300 ${open ? "rotate-180" : ""}`} />
      </button>
    ...
    <>
      ...
      <main>
        ...
        <section className="relative h-[200px] md:h-[260px] flex items-center justify-center overflow-hidden">
          ...
          <img src="/images/service-areas/hero.webp" alt="" aria-hidden="true" className="absolute inset-0 w-full h-full object-cover" style={{ objectPosition: "center 5%" }} />
        ...
        <section className="bg-[#F5F0E8] py-20 px-4">
          <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div ref={introRef} className="fade-up rounded-3xl overflow-hidden">
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src="/images/service-areas/intro.webp" alt="Lionz Roofing Service Area" className="w-full h-[420px] object-cover rounded-3xl" />
            </div>
            <div>
              ...
              <p className="text-gray-600 text-base leading-relaxed mb-6">
                From minor repairs to complete roof replacements, Lionz Roofing provides expert roofing solutions tailored to Southeastern Florida&apos;s unique climate. Whether you&apos;re a homeowner facing leaks or a business owner seeking long-term durability, our team delivers high-quality craftsmanship focused on protecting your property against intense sun, heavy rains, and hurricane threats. Every project we handle is built to last — backed by years of experience, trusted materials, and a commitment to customer satisfaction that truly sets us apart in the region.
              </p>
              <div className="flex flex-wrap gap-2 mb-8">
                {INTRO_PILLS.map((city) => (
                  <a key={city} href={`https://www.google.com/maps/search/roofing+contractor+${city.replace(/\s+/g, "+")},+FL`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 border border-lionzGold rounded-full px-4 py-2 hover:bg-lionzGold/10 transition-colors">
                    <MapPin size={12} className="text-lionzGold shrink-0" />
                    <span className="text-lionzNavy font-semibold text-sm">{city}</span>
                  </a>
                ))}
              </div>
              <Button href="/contact" variant="primary">Get Your Free Quote</Button>
            </div>
          </div>
        </section>
          ...
          <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div ref={solutionRef} className="fade-up">
              ...
              <p className="text-gray-600 text-base leading-relaxed mb-8">
                Lionz Roofing provides expert residential and commercial roofing across Miami, Fort Lauderdale, Boca Raton, West Palm Beach, and Hollywood. From durable, hurricane-ready roof replacements to commercial inspections, we protect homes and businesses from Florida&apos;s sun, storms, and heavy rains—delivering safe, code-compliant, and long-lasting results. Licensed, insured, and trusted across South Florida.
              </p>
            ...
            <div className="relative">
              <div className="rounded-3xl overflow-hidden">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src="/images/service-areas/solutions.webp" alt="Roofing solutions" className="w-full h-[420px] object-cover rounded-3xl" />
              </div>
              <div className="hidden sm:block absolute -bottom-6 -left-6 bg-lionzNavy rounded-2xl overflow-hidden shadow-2xl w-36 h-36 border-4 border-white">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src="/images/service-areas/team.webp" alt="Lionz Roofing team" className="w-full h-full object-cover" />
              </div>
            </div>
          </div>
        </section>
        ...
        <section className="bg-lionzLight py-20 px-4">
          <div className="max-w-7xl mx-auto">
            ...
            <div className="flex flex-wrap justify-center gap-3">
              {CITIES.map((city) => (
                <a key={city} href={`https://www.google.com/maps/search/roofing+contractor+${city.replace(/\s+/g, "+")},+FL`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 bg-white border border-gray-200 hover:border-lionzGold hover:bg-lionzGold/5 rounded-full px-5 py-2.5 transition-colors shadow-sm">
        ...
        <section className="bg-gray-50 py-20 px-4">
          <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
            <div ref={faqRef} className="fade-up lg:sticky lg:top-32">
              <div className="inline-flex items-center gap-2 border border-lionzGold text-lionzNavy text-xs font-bold uppercase tracking-widest px-5 py-2 rounded-full mb-6">FAQ</div>
              <h2 className="text-3xl md:text-4xl font-extrabold text-lionzDark leading-tight mb-4">
                Frequently Asked <span className="text-lionzGold">Questions</span>
              </h2>
              <p className="text-gray-500 text-base leading-relaxed">Have questions about our roofing services or coverage areas? Find answers to the most common questions below, or reach out to our team directly.</p>
              <div className="mt-8"><Button href="/contact" variant="outline">Ask Us Anything</Button></div>
            </div>
